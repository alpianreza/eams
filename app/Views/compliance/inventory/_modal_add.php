...
<div class="modal fade" id="modalAddInventory" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable"><div class="modal-content compliance-modal-content">
    <form action="<?= base_url('compliance/inventory/store') ?>" method="post" enctype="multipart/form-data" id="formAddInventory" onsubmit="return false;">
      <?= csrf_field() ?><input type="hidden" name="pic" id="add_pic">
      ...
      <div class="modal-body compliance-modal-body"><div class="row g-3">
        ...
        <div class="col-md-6"><label for="add_pic_primary" class="form-label">PIC Utama</label><select id="add_pic_primary" class="form-select" required><option value="">-- pilih user aktif --</option><?php foreach ($picUsers as $picUser): ?><option value="<?= esc($picUser['name']) ?>"><?= esc($picUser['name']) ?> · <?= esc($picUser['username']) ?></option><?php endforeach; ?></select></div>
        <div class="col-md-6"><label for="add_pic_secondary" class="form-label">PIC Kedua <span class="text-muted fw-normal">(opsional)</span></label><select id="add_pic_secondary" class="form-select"><option value="">-- tanpa PIC kedua --</option><?php foreach ($picUsers as $picUser): ?><option value="<?= esc($picUser['name']) ?>"><?= esc($picUser['name']) ?> · <?= esc($picUser['username']) ?></option><?php endforeach; ?></select><small class="text-muted">Maksimal 2 PIC dan harus user EAMS aktif.</small></div>
        ...
        <div class="col-12"><label for="add_photo" class="form-label">Foto Inventory</label><input id="add_photo" type="file" name="photo" class="form-control" accept="image/*"></div><div class="col-12 text-center"><img id="previewPhoto" class="img-fluid rounded d-none mt-2 inventory-add-preview" alt="Preview foto"></div>
      </div></div>
...
(function () {
  ...
  const primary = document.getElementById('add_pic_primary');
  const secondary = document.getElementById('add_pic_secondary');
  const hidden = document.getElementById('add_pic');
  if (!form || !primary || !secondary || !hidden) return;
  const sync = function () {
  ...
  primary.addEventListener('change', sync); secondary.addEventListener('change', sync); form.addEventListener('submit', sync); sync();
})();
</script>
