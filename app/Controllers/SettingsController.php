...
class SettingsController extends BaseController
{
  ...
  public function index()
  {
    ...
    return view('settings/index', ['title' => 'Pengaturan EAMS', 'settings' => $settings, 'currentUser' => $user ?? []]);
  }
  ...
  private function saveNotificationContact()
  {
    ...
    if ($email !== '' && ! filter_var($email, FILTER_VALIDATE_EMAIL)) return redirect()->back()->with('error', 'Alamat email tidak valid.');
  ...
  private function saveCompanySettings()
  {
    ...
    if ($logo && $logo->isValid() && ! $logo->hasMoved()) {
      ...
      $directory = FCPATH . 'uploads/company'; if (! is_dir($directory)) mkdir($directory, 0775, true);
      $name = $logo->getRandomName(); $logo->move($directory, $name); $model->put('company_logo', 'uploads/company/' . $name, false, $userId);
    }
