(function () {
  const page = document.querySelector('.fac-grid-page');
  if (!page) return;
  ...
  const setCellState = (cell, state) => {
    ...
    cell.innerHTML = '<span class="fac-cell-mark"></span>';
  };
  ...
  const saveCell = async (cell, mode) => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || 'Gagal menyimpan checklist First Aid Kit Content.');
      }

      setCellState(cell, result.state || (mode === 'clear' ? 'empty' : mode));
    } catch (error) {
      console.error(error);
      alert(error.message || 'Gagal menyimpan checklist First Aid Kit Content.');
    } finally {
      busyKey = null;
  ...
  const setAllEmptyCellsToOk = () => {
    page.querySelectorAll('.fac-check-cell').forEach((cell) => {
  ...
  const markAll = async () => {
    ...
    busyKey = '__bulk__';
    const button = page.querySelector('.fac-mark-all-btn');
    if (button) {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || 'Gagal mencentang semua First Aid Kit Content.');
      }

      setAllEmptyCellsToOk();
    } catch (error) {
      console.error(error);
      alert(error.message || 'Gagal mencentang semua First Aid Kit Content.');
    } finally {
      busyKey = null;
      if (button) {
  ...
  page.addEventListener('click', (event) => {
    const cell = event.target.closest('.fac-check-cell');
    if (cell) {
    ...
    const bulkButton = event.target.closest('.fac-mark-all-btn');
    if (bulkButton) {
      markAll();
    }
  });
