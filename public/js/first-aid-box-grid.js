(function () {
  const page = document.querySelector('.fab-grid-page');
  if (!page) return;
  ...
  const setCellState = (cell, state) => {
    ...
    cell.innerHTML = '<span class="fab-cell-mark"></span>';
  };
  ...
  const saveCell = async (cell, mode) => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || 'Gagal menyimpan checklist First Aid Box.');
      }

      setCellState(cell, result.state || (mode === 'clear' ? 'empty' : mode));
    } catch (error) {
      console.error(error);
      alert(error.message || 'Gagal menyimpan checklist First Aid Box.');
    } finally {
      busyKey = null;
  ...
  const markAll = async () => {
    ...
    busyKey = '__bulk__';
    const button = page.querySelector('.fab-mark-all-btn');
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || 'Gagal mencentang semua First Aid Box.');
      }

      page.querySelectorAll('.fab-check-cell').forEach((cell) => {
      ...
      console.error(error);
      alert(error.message || 'Gagal mencentang semua First Aid Box.');
    } finally {
      busyKey = null;
  ...
  page.addEventListener('click', (event) => {
    const cell = event.target.closest('.fab-check-cell');
    if (cell) {
      const state = cell.dataset.state || 'empty';
    ...
    const bulkButton = event.target.closest('.fab-mark-all-btn');
    if (bulkButton) {
      markAll();
    }
  });
