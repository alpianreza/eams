(function () {
  const page = document.querySelector('.el-grid-page');
  if (!page) return;
  ...
  const setCellState = (cell, state) => {
    ...
    cell.classList.remove('is-empty', 'is-ok', 'is-not-ok', 'is-na');
  ...
  const saveCell = async (cell, mode) => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || 'Gagal menyimpan checklist Emergency Light.');
      }

      setCellState(cell, result.state || (mode === 'clear' ? 'empty' : mode));
    } catch (error) {
      console.error(error);
      alert(error.message || 'Gagal menyimpan checklist Emergency Light.');
    } finally {
      busyKey = null;
